function checkPassword() {
  ...
  if (usenam == "admin" && pw.length < 13 && !pw.match(/[!@#$%^&*]{3}/)) {
    ...
    document.getElementById("message4").textContent="Password must be atleast 10 characters";
    document.getElementById('myform').reset();
    return false;

  } else if (pw.length < 10 && !pw.match(/[A-Za-z]/g) && pw.match(/[0-9]/g)) {
    document.getElementById("message5").textContent="Password must be atleast 10 characters and must include atleast one letter";
    document.getElementById('myform').reset();
    return false;
  
  } else if (pw.length < 10 && pw.match(/[A-Za-z]/g) && !pw.match(/[0-9]/g)) {
    document.getElementById("message6").textContent="Password must be atleast 10 characters and must include atleast one number";
    document.getElementById('myform').reset();
    return false;

  } else if (pw.length >= 10 && !pw.match(/[0-9]/g) && !pw.match(/[A-Za-z]/g)) {
    document.getElementById("message7").textContent="Password must include atleast one number and one number";
    document.getElementById('myform').reset();
    return false;

  } else if (pw.length >= 10 && !pw.match(/[0-9]/g) && pw.match(/[A-Za-z]/g)) {
    document.getElementById("message8").textContent="Password must include atleast one letter";
    document.getElementById('myform').reset();
    return false;

  } else if (pw.length >= 10 && pw.match(/[0-9]/g) && !pw.match(/[A-Za-z]/g)) {
    document.getElementById("message9").textContent="Password must include atleast one number";
    document.getElementById('myform').reset();
    return false;
...
function eraseText() {
  if (document.getElementsByTagName("span").value == "") {
    ...
    document.getElementsByTagName("span").empty();
  }
}
